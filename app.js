...
// ─── In-Memory API Cache (fixes 429 Too Many Requests) ───────────────────────
// Cache stores API responses for up to 1 hour to stay well within rate limits.
const _apiCache = new Map();
const API_CACHE_TTL = 60 * 60 * 1000; // 1 hour in ms
...
// Apply cache to all NASA/external API routes
app.use('/api/nasa', apiCacheMiddleware);
app.use('/api/weather', apiCacheMiddleware);
app.use('/api/asteroids', apiCacheMiddleware);
app.use('/api/space-weather', apiCacheMiddleware);
// ──────────────────────────────────────────────────────────────────────────────
